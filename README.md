...
## Public Media Next Step

The file `data/public_music_candidates.json` lists a first batch of public-safe candidate tracks from Pixabay Music. Download the MP3 files manually from the source pages, place them under `static/audio/public/`, and update `music.json` with the final `filepath`, `source_url`, `license`, and `attribution`.
