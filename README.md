...
The public repository includes a minimal runnable Flask demo. Original reports, slide decks, raw recordings, private identifiers, local environment details, and redistribution-sensitive media assets are not included. Additional public-safe demo evidence is documented in `docs/demo-evidence.md`.
...
公开仓库包含一个最小可运行的 Flask demo。原始报告、幻灯片、原始录屏、个人身份标识、本地环境细节和可能涉及再分发授权的媒体素材没有直接公开。脱敏后的 demo 证据记录在 `docs/demo-evidence.md`。
